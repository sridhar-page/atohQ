...
export default function MasterDashboard() {
  const [activeTab, setActiveTab] = useState<"patient" | "receptionist" | "admin" | "status" | "specialist">("receptionist");
...
const directVisitSchema = joinSchema.extend({
  isEmergency: z.boolean(),
});
type DirectVisitData = JoinFormData & { isEmergency: boolean };

function DirectVisitForm({ queueId, onSuccess }: { queueId: string, onSuccess: () => void }) {
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<DirectVisitData>({
    resolver: zodResolver(directVisitSchema),
    defaultValues: { isEmergency: false }
  });

  const onSubmit = async (data: DirectVisitData) => {
    try {
      await receptionistService.createToken({
        ...data,
        queueId: data.queueId || queueId // Use explicit clinic if selected, else default
      });
      reset();
      onSuccess();
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <input {...register("name")} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-emerald-500 outline-none transition-all font-bold text-sm" placeholder="Full Name" />
        ...
        <div className="space-y-1">
          ...
          <input {...register("phone")} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-emerald-500 outline-none transition-all font-bold text-sm" placeholder="Phone Number" />
          {errors.phone && <p className="text-[10px] text-rose-500 font-bold">{errors.phone.message}</p>}
        </div>
      </div>
      
      <div className="space-y-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Assigned Clinic</label>
        <select {...register("queueId")} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-emerald-500 outline-none transition-all font-bold text-sm appearance-none">
          <option value="">Select Service...</option>
          {queues.map(q => <option key={q.id} value={q.id}>{q.name}</option>)}
        </select>
        {errors.queueId && <p className="text-[10px] text-rose-500 font-bold">{errors.queueId.message}</p>}
      </div>
      ...
      <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
         <div className="flex items-center gap-3">
            ...
            <div>
               <p className="text-xs font-black text-slate-900">Emergency Protocol</p>
               <p className="text-[10px] font-bold text-slate-400">Bypass standard sequence</p>
            </div>
         </div>
         <input type="checkbox" {...register("isEmergency")} className="w-6 h-6 rounded-lg border-slate-200 text-emerald-600 focus:ring-emerald-500" />
      </div>

      <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-3">
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <CompactStat label="No Show Rate" value={stats?.efficiency || "0%"} icon={<Activity size={20} />} trend="daily" />
        <CompactStat label="Urgent Served" value={stats?.urgentCount || 0} icon={<AlertCircle size={20} />} trend="emergency" />
      </section>
        ...
        <section className="xl:col-span-5">
          <div className="glass p-10 rounded-[3rem] brand-glow space-y-8 bg-white/50 backdrop-blur-md border border-white shadow-xl">
             ...
             <div className="text-center space-y-4 py-10">
                {queueData.serving ? (
                  <>
                    <div className="text-8xl font-black text-slate-900 tracking-tighter transition-all">{queueData.serving.number}</div>
                    <div className="space-y-1">
                    ...
                    <button 
                       ...
                       className="w-full mt-6 py-5 bg-emerald-600 text-white rounded-3xl font-black text-lg shadow-2xl shadow-emerald-200 hover:bg-emerald-700 transition-all flex items-center justify-center gap-3"
                    >
                      <UserCheck size={24} />
                      COMPLETE & NEXT
                    </button>
              ...
              <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-3xl border border-slate-100 flex items-center justify-between group hover:border-emerald-200 transition-all shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    ...
                    className="bg-rose-50 text-rose-600 px-6 py-3 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-rose-600 hover:text-white transition-all"
                  >
                    NO SHOW
                  </button>
                </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="glass p-10 rounded-[3rem] bg-white border border-slate-100 space-y-8">
           <div className="space-y-1">
             <h3 className="text-2xl font-black text-slate-900 leading-none">Register Direct Visit</h3>
             <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Receptionist Control Panel</p>
           </div>
           <DirectVisitForm queueId={queueData.queueId} onSuccess={fetchData} />
        </div>

        <div className="glass p-10 rounded-[3rem] bg-slate-900 text-white border border-slate-800 space-y-8 h-full">
           <div className="space-y-1">
             <h3 className="text-2xl font-black italic">Service Intelligence</h3>
             <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Operational Protocol</p>
           </div>
           <div className="space-y-4">
              <div className="p-6 bg-white/5 rounded-3xl flex items-center justify-between">
                 <div>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Emergency Token Prefix</p>
                    <p className="text-xl font-black tracking-tighter text-amber-400">E-IDENTIFIER</p>
                 </div>
                 <ShieldCheck className="text-amber-400" size={32} />
              </div>
              <p className="text-xs text-slate-400 font-bold italic leading-relaxed">Direct visitors and emergency cases bypass the standard digital queue sequence to ensure immediate medical attention when prioritized by the front desk.</p>
           </div>
        </div>
      </section>
    </div>
...
function SpecialistView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setMetrics(m);
        // Filter for General Medicine only
        setDepartments(d.filter(dept => dept.name === "General Medicine"));
        setInsights(i);
      } catch (err) {
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-slate-900 leading-none tracking-tight">Specialist Intelligence</h1>
          <p className="text-slate-400 font-bold uppercase text-xs tracking-widest">Departmental Performance Control</p>
        </div>
      </header>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
         <BigStat icon={<Users className="text-emerald-500" />} label="Visited (Dept)" value={metrics?.visitedToday?.toString() || "0"} trend="Live" />
         <BigStat icon={<CheckCircle2 className="text-blue-500" />} label="Served (Dept)" value={metrics?.servedToday?.toString() || "0"} trend="Active" />
         <BigStat icon={<Activity className="text-amber-500" />} label="In Queue" value={metrics?.waitingToday?.toString() || "0"} trend="Current" />
         <BigStat icon={<ShieldCheck className="text-indigo-500" />} label="Service Status" value="Online" trend="Stable" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <section className="lg:col-span-8 space-y-6">
          <div className="glass p-8 rounded-[3rem] brand-glow bg-white border border-slate-100 space-y-8">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-2xl font-black text-slate-900 leading-none">Department Traffic</h3>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Volume Analysis</p>
              </div>
            </div>
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={insights?.hourlyFlow || []}>
                  <defs>
                    <linearGradient id="colorFlowSpec" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#10b981" stopOpacity={0.3}/>
                      <stop offset="95%" stopColor="#10b981" stopOpacity={0}/>
                    </linearGradient>
                  </defs>
                  ...
                  <Tooltip 
                    contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontWeight: 900 }}
                  />
                  <Area type="monotone" dataKey="count" stroke="#10b981" strokeWidth={4} fillOpacity={1} fill="url(#colorFlowSpec)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-4">
             <div className="glass p-8 rounded-[2.5rem] bg-white border border-slate-100 space-y-6">
                <h4 className="text-lg font-black text-slate-900">Efficiency Metrics</h4>
                <div className="space-y-6">
                   <div className="space-y-2">
                      <div className="flex justify-between items-end">
                         <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Avg Consult Time</span>
                         <span className="text-lg font-black text-emerald-600">{insights?.serviceStats.avg}m</span>
                      </div>
                      <div className="h-2 bg-slate-50 rounded-full overflow-hidden">
                         <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(100, (insights?.serviceStats.avg || 0) * 5)}%` }} className="h-full bg-emerald-500 rounded-full" />
                   ...
                   <div className="space-y-2">
                      <div className="flex justify-between items-end">
                         <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Peak Session</span>
                         <span className="text-lg font-black text-slate-900">{insights?.serviceStats.max}m</span>
                      </div>
                      <div className="h-2 bg-slate-50 rounded-full overflow-hidden">
                         <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(100, (insights?.serviceStats.max || 0) * 2)}%` }} className="h-full bg-slate-900 rounded-full" />
             ...
             <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                  <h3 className="text-xl font-black text-slate-900">Active Department</h3>
                </div>
                <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
                  ...
                  {departments.length === 0 && (
                    <div className="p-5 text-center text-slate-400 font-bold text-xs uppercase italic">Department not active</div>
                  )}
                </div>
             </div>
          </div>
        ...
        <section className="lg:col-span-4 space-y-6">
           <div className="glass p-8 rounded-[2.5rem] brand-glow bg-slate-900 text-white border border-slate-800 space-y-8 h-full">
              <div>
                <h4 className="text-lg font-black italic">Dept Insights</h4>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Specialist Performance</p>
              </div>
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-emerald-400"><Users size={20} /></div>
                      <div>
                         <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Queue Load</p>
                         <p className="text-lg font-black leading-none">{departments[0]?.flow || "0 Tokens"}</p>
                      </div>
                   </div>
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-amber-400"><Clock size={20} /></div>
                      <div>
                         <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Avg Wait</p>
                         <p className="text-lg font-black leading-none">{departments[0]?.wait || "0 mins"}</p>
                      </div>
                   </div>
                </div>

                <button className="w-full py-4 bg-emerald-600 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all">Submit Dept Logs</button>
              </div>
           </div>
        </section>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="space-y-2">
              ...
              <select {...register("queueId")} className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-emerald-500 outline-none transition-all font-bold text-slate-900 appearance-none">
                <option value="">Select Service...</option>
                {queues.map(q => <option key={q.id} value={q.id}>{q.name} ({q.currentWait})</option>)}
              </select>
